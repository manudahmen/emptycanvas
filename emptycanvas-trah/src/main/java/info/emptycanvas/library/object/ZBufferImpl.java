...
public class ZBufferImpl implements ZBuffer {
    ...
    public void suivante() {
        ...
        if (original == null) {
            ...
            int c = COULEUR_FOND.getRGB();
            for (int i = 0; i < original.length; i++) {
                original[i] = c;
            }

            couleurDeFond(COULEUR_FOND);
        }
        Sc = Arrays.copyOf(original, la * ha);
    }
    ...
    public void couleurDeFond(TColor c) {
        ...
        if (c.type() == TColor.TYPE_COULEUR) {
            ...
            for (int x = 0; x < la; x++) {
                for (int y = 0; y < ha; y++) {
                    original[x + y * la] = c.getColorAt(1.0 * x / la, 1.0 * y / ha);
                }
            }
        }
    }
    ...
    public class ImageMapElement {
        ...
        private void setRGBInts(Color rgb, int[] sc, int x, int y) {
            /*
             sc[(x + y * la) * 3 + 0] = rgb.getRed();
             sc[(x + y * la) * 3 + 1] = rgb.getGreen();
             sc[(x + y * la) * 3 + 2] = rgb.getBlue();
             */
            sc[x + y * la] = 0xFF000000 | rgb.getRed() << 16 | rgb.getGreen() << 8 | rgb.getBlue();
        }
        ...
        public int COULEUR_FOND_INT() {
            couleur_fond_int = COULEUR_FOND.getRGB();
        ...
        private int getRGBInt(int[] sc, int x, int y) {
            ...
            /*
             return sc[(x + y * la) * 3 + 0] << 16
             | sc[(x + y * la) * 3 + 1] << 8 | sc[(x + y * la) * 3 + 2];
             */
            return sc[x + y * la];
    ...
    public class ImageMap {
        ...
        public void testProf(Point3D x3d, Point coordonneesPointEcra, Color c) {
            ...
            if (x >= 0 & x < la & y >= 0 & y < ha
                ...
                if (scene().lumiereActive() != null) {
                    c = scene().lumiereActive().getCouleur(c, x3d, x3d.getNormale() != null ? x3d.getNormale() : null);
                    c = scene().calculerCouleurLumiere(c, x3d, x3d.getNormale() != null ? x3d.getNormale() : null);
                }
        ...
        public void testProf(Point3D x3d, Color c) {
            ...
            if (x >= 0
                ...
                ime.setElementPoint(x, y, x3d);
                if (scene().lumiereActive() != null) {
                    c = scene().lumiereTotaleCouleur(c, x3d, x3d.getNormale() != null ? x3d.getNormale() : null);
                }
                ime.setElementCouleur(x, y, c);
                ime.setProf(x, y, prof);
            ...
            if (r instanceof TRIObject) {
                ...
                while (ts.hasNext()) {
                    ...
                    tracerTriangle(t.getSommet()[0], t.getSommet()[1],
                            t.getSommet()[2], new Color(t.texture().getColorAt(0.5,0.5)));
                }
            } else if (r instanceof BSpline) {
                BSpline b = (BSpline) r;
                int nt = 100;
    ...
    public void dessinerSilhouette() {
        ...
        while (it.hasNext()) {
            ...
            if (r instanceof TRIObject) {
                ...
                for (TRI t : ((TRIConteneur) r).iterable()) {
                    // pi = new TrianglePix();
                    tracerAretes(t.getSommet()[0], t.getSommet()[1],
                            new Color(t.texture().getColorAt(0.5,0.5)));
                    tracerAretes(t.getSommet()[1], t.getSommet()[2],
                            new Color(t.texture().getColorAt(0.5,0.5)));
                    tracerAretes(t.getSommet()[2], t.getSommet()[0],
                            new Color(t.texture().getColorAt(0.5,0.5)));
                    // tracerTriangle(pi);
        ...
        if (re instanceof RepresentableConteneur) {
            ...
            if (r instanceof TRIObject) {
                ...
                dessinerStructure(new SegmentDroite(camera(t.getSommet()[0]),
                        camera(t.getSommet()[1]), new Color(t.texture().getColorAt(0.5,0.5))));
                dessinerStructure(new SegmentDroite(camera(t.getSommet()[1]),
                        camera(t.getSommet()[2]), new Color(t.texture().getColorAt(0.5,0.5))));
                dessinerStructure(new SegmentDroite(camera(t.getSommet()[2]),
                        camera(t.getSommet()[0]), new Color(t.texture().getColorAt(0.5,0.5))));
            } else if (r instanceof BSpline) {
                BSpline b = (BSpline) r;
                int nt = 100;
                for (double i = 0; i < nt; i++) {
    ...
    public void tracerTriangle(Point3D pp1, Point3D pp2, Point3D pp3, Color c) {
        ...
        /*
         * pp1 = cameraC.calculerPointDansRepere(pp1); pp2 =
         * cameraC.calculerPointDansRepere(pp2); pp3 =
         * cameraC.calculerPointDansRepere(pp3);
         */
    ...
    public ECBufferedImage image() {
        ...
        for (int i = 0; i < la; i++) {
            for (int j = 0; j < ha; j++) {
                ...
                /*
                 * Graphics g = bi2.getGraphics(); g.setColor(new
                 * Color(ime.getIME().getElementCouleur(i, j))); g.drawLine(i,
                 * j, i, j);
                 */
            }
        }
    ...
    public void testPoint(Point3D p, Color c) {
        if (scene().lumiereActive() != null) {
            c = scene().lumiereActive().getCouleur(c, p, p.getNormale());
        }
        if (p != null && c != null) {
            ime.testProf(p, c);
        }
    ...
    public Point3D camera(Point3D p) {
        if (this instanceof ZBuffer3DImpl
                && currentScene.cameraActive() instanceof Camera3D) {
            if ((((ZBuffer3DImpl) this).LR()) == 0) {
                Point3D p2 = ((Camera3D) currentScene.cameraActive())
                        .calculerPointDansRepereGAUCHE(p);
                if (p.getNormale() != null) {
                    p2.setNormale(((Camera3D) currentScene.cameraActive())
                            .calculerPointDansRepereGAUCHE(p.getNormale()));
                }
                return p2;
            }
            if (((ZBuffer3DImpl) this).LR() == 1) {
                Point3D p2 = ((Camera3D) currentScene.cameraActive())
                        .calculerPointDansRepereDROIT(p);
                if (p.getNormale() != null) {
                    p2.setNormale(((Camera3D) currentScene.cameraActive())
                            .calculerPointDansRepereDROIT(p.getNormale()));
                }
                return p2;
            }
        }
    ...
    public Color getColorAt(Point p) {
        if(ime.getIME().getElementProf((int) p.getX(),
                (int) p.getY())>=INFINI_PROF)
    ...
    public Representable getObjectAt(Point p) {
        if(ime.getIME().getElementProf((int) p.getX(),
                (int) p.getY())>=INFINI_PROF)
            return ime.getIME().getElementRepresentable((int)p.getX(), (int)p.getY());
        else
            return null;
    }
    @Override
