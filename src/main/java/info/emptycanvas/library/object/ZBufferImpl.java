			...
			if (r instanceof TRIObject) {
				...
				for (double i = 0; i < 1; i += incr) {
					for (double j = 0; j < 1; j += incr) {
						TRI[] tris = new TRI[2];
						tris[0] = new TRI(n.formule(i, j), n.formule(i + incr,
								j), n.formule(i + incr, j + incr), new TColor(
								Color.WHITE));
						tris[1] = new TRI(n.formule(i + incr, j + incr),
								n.formule(i, j + incr), n.formule(i, j),
								new TColor(Color.WHITE));
